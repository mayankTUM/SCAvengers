...
public class RegisterService {
	...
	public void registerService(String authToken, List<CreateBusinessServicePOJO> list) throws DispositionReportFaultMessage, RemoteException, TransportException {

		String businessEntityKey = createBusinessEntity(authToken, list.get(0));
		System.out.println("created BusinessEntity:  " + businessEntityKey);

		Iterator<CreateBusinessServicePOJO> iter = list.iterator();
		
		while(iter.hasNext())
		{
			CreateBusinessServicePOJO config = iter.next();
...
//			System.out.println("created BusinessEntity:  " + businessEntityKey);
			...
			des.setValue(config.getDescription());
			businessService.getDescription().add(des);
			businessService.setBusinessKey(businessEntityKey); //group name
			Name myServName = new Name();
			myServName.setValue(config.getBusinessEntityName());
			...
			String bindingKey = createBinding(authToken, config, businessService, serviceKey);		    
			...
			UDDIPublicationPortType uddiPublish2 = transport.getUDDIPublishService();
			ServiceDetail sd = uddiPublish2.saveService(updates); //update the Service
			serviceKey = sd.getBusinessService().get(0).getServiceKey();
			System.out.println("updated Business Service:  " + serviceKey);
		}
	}

	private String createBinding(String authToken, CreateBusinessServicePOJO config, BusinessService myService, String myServiceKey) throws TransportException, DispositionReportFaultMessage, RemoteException {
		...
		Description des = new Description();
		des.setValue(config.getDescription());
		myTemplate.getDescription().add(des);
		...
		for(TModel tm: tModelList) {
			...
			instanceDetails.setInstanceParms("Helloworld tModel instance");
		...
		AccessPoint helloworldAccessPoint = createAccessPoint(config.getEndpoint());
		...
		String bindingKey = binddetail.getBindingTemplate().get(0).getBindingKey();
		return bindingKey;
	}
	 ...
	 */
	private String createBusinessService(String authToken, BusinessService myService) throws TransportException, DispositionReportFaultMessage, RemoteException {
	 ...
	 */
	private String createBusinessEntity(String authToken, CreateBusinessServicePOJO config) throws DispositionReportFaultMessage, RemoteException, TransportException {
	 ...
	 *
	 */
	private TModelDetail createtModel(String authinfo, CreateBusinessServicePOJO config) throws TransportException, DispositionReportFaultMessage, RemoteException {
		...
		List<OverviewDoc> overviewdoclist = helloworldWSDL_tModel.getOverviewDoc();
